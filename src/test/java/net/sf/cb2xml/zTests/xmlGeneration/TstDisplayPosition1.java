...
//		writeItms(copybook.getChildItems());
	...
	private Lengths checkItms(Lengths l, List<? extends IItem> items, List<ItmDtls> expected) {
		...
		for (int i = 0; i < items.size(); i++) {
			...
			assertEquals(l.storage, item.getPosition(), item.getFieldName());
			assertEquals(l.display, item.getDisplayPosition(), item.getFieldName());
			...
			if (itmDtls.children != null && itmDtls.children.size() > 0) {
				...
				assertEquals(tmp.storage, ll.storage, item.getFieldName());
				assertEquals(tmp.display, ll.display, item.getFieldName());
			} 
			int occurs = 1;
			if (item.getOccurs() >= 0) {
